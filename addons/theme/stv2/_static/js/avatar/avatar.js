...
var avatar = function( args ) {
	...
	var uploadForm = args.uploadForm,
		...
		set_UP_W_H = function( w, h ){
			...
			zbq_border = up_pic_height > up_pic_width ? up_pic_width : up_pic_height;
			if(up_pic_height > up_pic_width){
				zbq_x1 = 0;
				zbq_y1 = (up_pic_height-up_pic_width)/2;
			} else {
				zbq_x1 = (up_pic_width-up_pic_height)/2;
				zbq_y1 = 0;
			}
                	...
                	if ( 1 == txt.status ) {
                		...
                		M.getJS( THEME_URL + "/js/imgareaselect/jquery.imgareaselect.min.js", function() {
				            ...
				            imgrs = $area_img.imgAreaSelect({ 
		                        x1: zbq_x1, 
		                        y1: zbq_y1,
		                        x2: parseInt(zbq_x1)+parseInt(zbq_border), //初始矩形宽
		                        y2: parseInt(zbq_y1)+parseInt(zbq_border), //初始矩形高
		                        handles: true,
		                        onInit: previewFn,
		                    ...
		                    $area_img.load(function(){
						    	imgrs.setSelection(zbq_x1, zbq_y1, parseInt(zbq_x1)+parseInt(zbq_border), parseInt(zbq_y1)+parseInt(zbq_border), true); 
						    	imgrs.setOptions({ show: true });
        						imgrs.update();  
						    })
				            // 隐藏上传表单，显示设置表单
