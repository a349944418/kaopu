...
class UserModel extends Model {
	...
	public function getUserInfo($uid) {
		...
		$user['postCount'] = D('weiba_post')->where('post_uid='.$uid.' and is_del=0 and post_type=1')->count();
		$user['shareCount'] = D('weiba_post')->where('post_uid='.$uid.' and is_del!=1 and post_type=2')->count();
		$user['replyCount'] = D('weiba_reply')->where('is_del=0 and uid='.$uid)->count();
		$user['favoriteCount'] = D('weiba_favorite')->where('uid='.$uid)->count();
		$user['followCount'] = D('weiba_follow')->where('follower_uid='.$uid)->count();

		$follow = D('weiba_follow')->where('follower_uid='.$uid)->findAll();
		$map1['weiba_id'] = array('in', getSubByKey($follow, 'weiba_id'));
		$map1['is_del'] = 0;
		$weibaList = D('weiba')->field('weiba_id, weiba_name, logo')->where($map1)->findAll();
		foreach($weibaList as $k=>$v){
			$weibaList[$k]['logo'] = getImageUrlByAttachId($v['logo'],200,200);
		}
		$user['weibaList'] = $weibaList;

		static_cache ( 'user_info_' . $uid, $user );
		static_cache ( 'ui_' . $uid, $user );
	...
	public function getUserInfoByUids($uids) {
		...
		foreach ( $uids as $v ) {
			$cacheList [$v] = $this->getUserInfo ( $v );
		}
