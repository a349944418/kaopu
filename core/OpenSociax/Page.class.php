...
{//类定义开始
     ...
     * @return string
     */
    public function zshow($isArray=false){
        ...
        if ($downRow <= $this->totalPages){
            $downPage="<a href='".$url."&".C('VAR_PAGE')."=$downRow' class='next'>换一批</a>";
        }else{
            $downPage="<a href='".$url."&".C('VAR_PAGE')."=1' class='next'>换一批</a>";
        }

        $pageStr = $this->totalPages == 1 ? '' : $downPage;
        return $pageStr;
