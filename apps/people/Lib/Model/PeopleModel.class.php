...
class PeopleModel extends model
{
	...
	public function getPeople($data, $type)
	{
		...
		switch($type) {
			...
			case 'official':
				...
				break;
			case 'indexOfficial':
				$list = $this->_getOfficialData($data, $field = 'u.uid, v.info', $order = 'u.uid DESC', $page = 4, $zwhere = 1);
				break;
		}
		// 获取用户ID
	 ...
	 * @param string $order 排序数据
	 * @param integer $page 分页个数
	 * @param integer $zwhere 标识符,代表来自首页
	 * @return array 筛选官方用户数据列表
	 */
	private function _getOfficialData($data, $field = 'u.uid, o.info', $order = 'u.uid DESC', $page = 30, $zwhere = 0)
	{
		// 设置表明
		...
		// 查询数据
		if($zwhere)
			$list = D()->table($table)->where($map)->order($order)->findPage($page,$count=false,$options=array(),$zwhere=1);
		else
			$list = D()->table($table)->where($map)->order($order)->findPage($page);
