...
class RegisterAction extends Action
{
	...
	public function ajaxSchool() {
		...
		$this->assign('id', $id);
		if( $t == 'middle' || $t == 'high') {
			$province = M('area')->field('area_id,title')->where('pid=0')->order('sort')->select();
		} elseif ( $t == 'university') {
			$province = M('univsArea')->field('univs_area_id as area_id, title')->where('pid=0')->order('sort')->select();
		}
...
*/
	...
	public function step3() {
		...
		$this->appCssList[] = 'login.css';
		$this->assign('interester_open',$this->_config['interester_open']);
		$this->setTitle('完善个人信息');
		$this->setKeywords('完善个人信息');
		$this->display();
	}
